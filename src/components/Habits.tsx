...
export default function Habits(props: IHabitsProps) {
  ...
  const [habits, setHabits] = useState<ITableProps>(enrichHabits(stagedData, currentDate, lockFuture, lockPast));

  const loadData = useCallback(()=>{
    // TODO get the jsonData from cookie
    setHabits(enrichHabits(stagedData, currentDate, lockFuture, lockPast))
  }, [])
...
function fillBlanks(row: IRowProps, today:number, backwards:number, forwards: number): IRowProps {
  ...
  for (let d = start; d <= end; d += daySeconds) {
    const existing = row.activities.find((c: ICheckboxProps)=>{return c.date == d})
    if (existing) {
